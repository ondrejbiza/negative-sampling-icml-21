...
class StateTransitionsDatasetNegs(StateTransitionsDataset):
    ...
    def __init__(self, hdf5_file, in_ep_prob, time_aligned=True):

        super(StateTransitionsDatasetNegs, self).__init__(hdf5_file)
        self.in_ep_prob = in_ep_prob
        self.time_aligned = time_aligned
        assert 0 <= self.in_ep_prob <= 1
    ...
    def __getitem__(self, idx):
        ...
        in_out = np.random.choice([True, False], p=[self.in_ep_prob, 1 - self.in_ep_prob])
        ...
        if in_out:
            ...
            while True:
                ...
                if random_step != step:
                    # make sure our pos-neg state pair is not the exact same state
                    break
            ...
            if not self.time_aligned:
                random_step = np.random.randint(len(self.experience_buffer[random_ep]['obs']))

            neg_obs = to_float(self.experience_buffer[random_ep]['obs'][random_step])

        return obs, action, next_obs, neg_obs


class StateTransitionsDatasetNegsMany(StateTransitionsDataset):

    def __init__(self, hdf5_file, in_ep_prob, time_aligned=True, num_negs=1):

        super(StateTransitionsDatasetNegsMany, self).__init__(hdf5_file)
        self.in_ep_prob = in_ep_prob
        self.time_aligned = time_aligned
    ...
    def __getitem__(self, idx):
        ...
        for neg_idx in range(self.num_negs):
            ...
            neg_obs_list.append(neg_obs)

        neg_obs = np.stack(neg_obs_list, axis=0)
